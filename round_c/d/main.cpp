...
void solve (int iter) {
	...
	readll(MINIMUM) readll(MAXIMUM) readll(MEAN) readll(MEDIAN)
	ll res = -1;


	if (MINIMUM == MAXIMUM && MEAN == MINIMUM && MEDIAN == MINIMUM) {
		res = 1;

	} else if (
			MINIMUM > MAXIMUM ||
			MEDIAN > MAXIMUM || MEDIAN < MINIMUM ||
			MEAN > MAXIMUM || MEAN < MINIMUM ||
			MAXIMUM > MINIMUM && (MEAN == MINIMUM || MEAN == MAXIMUM)) {
	} else {
		rep(n,2,1e7) {
			if (n == 2 && MINIMUM + MAXIMUM == MEDIAN * 2 && MEAN * 2 == MINIMUM + MAXIMUM) {
				res = 2;
				break;
			}
			debug cerr << "n = " << n << endl;
			...
			ll k = (n - 3) / 2;
			ll sum0 = MINIMUM + MAXIMUM + MEDIAN * (n%2==1? 1 : 2);
			ll sum = MEAN * n;
			ll need = sum - sum0;
			ll sumLo = k * (MINIMUM + MEDIAN);
			ll sumHi = k * (MEDIAN + MAXIMUM);
			if (sumLo <= need && sumHi >= need) {res = n; break;}
		}
	}
