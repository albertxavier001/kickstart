...
void solve (int iter) {
	...
	vector<string> a(N+1);
	veci s(N);
	new_cubei(dp, Q+1, Q+1, Q+1, -1);

	rep(i,0,N+1) {cin>>a[i];}
	rep(i,0,N) {cin>>s[i];}

	if (N == 1) { a.push_back(a[a.size()-1]); a[1] = a[0]; s.push_back(s[s.size()-1]); s[1]=s[0]; N=2; }
	dp[0][0][0] = 0;
	rep (q,0,Q) rep(i,0,Q) rep(j,0,Q) if (dp[q][i][j] >= 0) for (auto x: {'T', 'F'}) {
						auto &tmp = dp[q+1][i + (a[0][q]==x)][j + (a[1][q]==x)];
						tmp = max(tmp, dp[q][i][j]+ (x==a[2][q]) );
					}
